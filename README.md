...
- **Datos**: dos Excel viven en el repo — `data/matriz_criticidad.xlsx` (las 602 refacciones críticas, cambia poco) y `data/ZMADE.xlsx` (export diario de SAP). Un script los cruza y genera `data/maestro.json`.
...
- **Actualización diaria**: subes el nuevo `ZMADE.xlsx` al repo y un GitHub Action hace el cruce automáticamente.

**Ya viene con tus datos reales cargados** (matriz de 602 refacciones + tu export ZMADE del 27/07/2026), así que en cuanto lo subas y actives Pages, ya vas a ver la tabla funcionando.

## 1. Subir este repo a GitHub

Sube todo el contenido de esta carpeta tal cual a tu repositorio.

⚠️ `data/ZMADE.xlsx` pesa ~6.6 MB — GitHub lo acepta sin problema (el límite normal es 100 MB por archivo), solo puede tardar un poco más al subirlo.
...
1. Exporta el ZMADE de SAP como siempre.
2. En GitHub, sube ese archivo reemplazando `data/ZMADE.xlsx` (Add file → Upload files, arrastra el nuevo archivo — GitHub detecta que es el mismo nombre y lo sobrescribe).
